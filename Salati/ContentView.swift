    ...
    var body: some View {
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 18) {
                    HeaderView(
                        hijriDate: vm.hijriDate,
                        hijriMonthYear: vm.hijriMonthYear,
                        gregorianDate: vm.gregorianDate
                    )
                    
                    NextPrayerCard(
                        nextPrayer: vm.nextPrayer,
                        timeRemaining: vm.timeRemaining,
                        progress: vm.progressToNextPrayer
                    )
                    
                    // أزرار سريعة
                    HStack(spacing: 10) {
                        QuickButton(icon: "location.fill", title: "مكة", subtitle: "21.38°N") {}
                        QuickButton(icon: "bell.badge.fill", title: "التنبيهات", subtitle: notifStatus, action: {
                            handleNotificationTap()
                        })
                        QuickButton(icon: "compass.drawing", title: "القبلة", subtitle: "21°") {
                            showQibla.toggle()
                        }
                    }
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        ForEach(vm.prayers) { prayer in
                            PrayerRow(
                                prayer: prayer,
                                isNext: prayer.id == vm.nextPrayer?.id,
                                isCurrent: prayer.id == vm.currentPrayer?.id && prayer.id != vm.nextPrayer?.id
                            )
                        }
                    }
                    .padding(14)
                    ...
                    // إعدادات الأذان
                    Button {
                        showAdhanSettings = true
                    } label: {
                        HStack(spacing: 12) {
                            ZStack {
                                RoundedRectangle(cornerRadius: 12)
                                    .fill(Color(hex: "#1B8B6A"))
                                    .frame(width: 44, height: 44)
                                Text(adhanMgr.selected.icon)
                                    .font(.title3)
                            }
                            VStack(alignment: .leading, spacing: 3) {
                                Text("صوت الأذان")
                                    ...
                                    .foregroundColor(.white)
                                Text(adhanMgr.selected.nameAr + " • " + (adhanMgr.isEnabled ? "مفعل" : "صامت"))
                                    .font(.caption2)
                    ...
                    VStack(spacing: 8) {
                        ...
                        Text("«إنَّ الصَّلاةَ كَانَتْ عَلَى الْمُؤْمِنِينَ كِتَابًا مَّوْقُوتًا»")
        ...
        .sheet(isPresented: $showAdhanSettings) {
            NavigationView { AdhanSettingsView() }
        }
        ...
        UNUserNotificationCenter.current().getNotificationSettings { settings in
            DispatchQueue.main.async {
                ...
                case .authorized, .provisional:
                    // إعادة جدولة وإرسال تجربة
                    PrayerService.scheduleNotifications(for: vm.prayers)
                    notifAlertMsg = "الإشعارات مفعلة ✅\nتمت إعادة جدولة كل الصلوات.\nستصلك: \"حان وقت صلاة الفجر\" ... وهكذا لكل صلاة في وقتها تماماً."
                    showNotifAlert = true
                    // اختبار اختياري: أرسل إشعار بعد 5 ثواني للتجربة
                    // PrayerService.sendTestNotification(after: 5)
                default:
                    break
                }
            }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    Text("في مكة المكرمة أنت في الحرم 🕋")
                        .font(.subheadline).bold().foregroundColor(.white)
                    Text("اتجه نحو الكعبة المشرفة مباشرة. التطبيق يستخدم موقعك الحالي لحساب الاتجاه الدقيق في النسخة الكاملة.")
